...
// ── Thread/todo model ─────────────────────────────────────────────
// Each PR has a `threads` array. A thread starts with a reviewer
// comment (or review-with-body) and may have replies. Each message
// has `done: bool` — a non-mine message is a todo for me to address.
// Replies from me have `isMine: true` and never count as todos.
//
// A thread is "resolved" when every non-mine message in it has
// `done: true`. A new non-mine reply arriving in a resolved thread
// flips it back to open.
//
// Helpers below derive counts from this shape.

function threadIsResolved(thread) {
  return thread.messages.every(m => m.isMine || m.done);
}
function threadHasNew(thread) {
  return thread.messages.some(m => !m.isMine && !m.done && m.isNew);
}
function threadOpenCount(thread) {
  return thread.messages.filter(m => !m.isMine && !m.done).length;
}
function prTodoCounts(pr) {
  const threads = pr.threads || [];
  const total = threads.length;
  const done = threads.filter(threadIsResolved).length;
  const openMessages = threads.reduce((s, th) => s + threadOpenCount(th), 0);
  return { total, done, open: total - done, openMessages };
}
...
// ── Threads attached to each PR ────────────────────────────────────
// Authored in chronological order. The detail view will split into
// "Open" and "Resolved" sections.

function attachThreads() {
  const byId = (id) => PULL_REQUESTS.find(p => p.id === id);

  byId(5107).threads = [
    {
      id: 'th-5107-1',
      where: 'BadgeFetcher.swift L142',
      kind: 'review-comment',
      messages: [
        { id: 'm1', actor: 'danieldraper', at: '2026-04-21T18:22:00Z',
          body: 'Tiny nit: prefer `guard let` over the if-let pyramid here — we do it everywhere else in this file.',
          done: true },
        { id: 'm2', actor: 'alex.chen', at: '2026-04-21T22:10:00Z', isMine: true,
          body: 'Fixed in c88a013.' },
      ],
    },
    {
      id: 'th-5107-2',
      where: 'BadgeFetcher.swift L218',
      kind: 'review-comment',
      messages: [
        { id: 'm3', actor: 'danieldraper', at: '2026-04-21T18:24:00Z',
          body: 'Could we extract the fallback fetch into its own helper so the test reads cleaner?',
          done: true },
        { id: 'm4', actor: 'alex.chen', at: '2026-04-22T08:11:00Z', isMine: true,
          body: 'Extracted `resolveByName(_:issuer:)` — see c88a013.' },
        { id: 'm5', actor: 'danieldraper', at: '2026-04-22T09:02:00Z',
          body: 'Perfect.', done: true },
      ],
    },
    {
      id: 'th-5107-3',
      ...
      kind: 'pr-comment',
      messages: [
        { id: 'm6', actor: 'theconstellationiris', at: '2026-04-22T08:45:00Z',
          body: "Hmm, I'm not seeing the badge image load on the device for users who migrated from v4.2. Are we handling the case where `issuerId` is also nil? Repro steps in our QA channel.",
          done: true },
        { id: 'm7', actor: 'alex.chen', at: '2026-04-22T14:12:00Z', isMine: true,
          body: 'Good catch — added a catalog lookup fallback. Pushed c88a013.' },
      ],
    },
    {
      id: 'th-5107-4',
      where: 'BadgeCatalog.swift L64',
      kind: 'review-comment',
      kindLabel: 'Changes requested',
      messages: [
        { id: 'm8', actor: 'theconstellationiris', at: '2026-04-22T20:01:00Z',
          body: "Catalog lookup is good but I'm worried about the perf cost on the shelf view where we already have 20+ badges visible. Can we batch the fetch or cache by issuerId?",
          done: true },
        { id: 'm9', actor: 'alex.chen', at: '2026-04-23T07:32:00Z', isMine: true,
          body: 'Batched + memoized per-session in d4f91ee/e8b03c1.' },
        { id: 'm10', actor: 'theconstellationiris', at: '2026-04-23T10:42:00Z',
          body: 'Testing out on device now!', done: false, isNew: true },
      ],
    },
    {
      id: 'th-5107-5',
      where: 'BadgeFetcherTests.swift L88',
      kind: 'review-comment',
      messages: [
        { id: 'm11', actor: 'theconstellationiris', at: '2026-04-23T10:44:00Z',
          body: "One more — mind adding a regression test for the migrated-from-v4.2 path while you're here?",
          done: false, isNew: true },
      ],
    },
  ];

  // Smaller thread sets for the other PRs so the source list shows variety
  byId(5072).threads = [
    {
      id: 'th-5072-1', where: 'PlayerView.swift L142', kind: 'review-comment',
      messages: [
        { id: 'a1', actor: 'danieldraper', at: '2026-04-22T15:00:00Z',
          body: 'Beautiful. Approving.', done: true },
      ],
    },
    {
      id: 'th-5072-2', where: 'general', kind: 'pr-comment',
      messages: [
        { id: 'a2', actor: 'marisol.v', at: '2026-04-22T16:30:00Z',
          body: "Looks great on the design side. Could we land this before the release branch cut on Friday?",
          done: true },
        { id: 'a3', actor: 'alex.chen', at: '2026-04-22T16:32:00Z', isMine: true,
          body: "That's the plan — just waiting on the second approval." },
      ],
    },
  ];
  ...
  byId(5114).threads = [
    {
      id: 'th-5114-1', where: 'general', kind: 'pr-comment',
      messages: [
        { id: 'b1', actor: 'kenji', at: '2026-04-23T08:02:00Z',
          body: "Alex — can you eyeball the throttle threshold? I picked 150ms but you'd know better from the analytics side.",
          done: false, isNew: true },
      ],
    },
    {
      id: 'th-5114-2', where: 'AnalyticsThrottler.swift L31', kind: 'review-comment',
      messages: [
        { id: 'b2', actor: 'kenji', at: '2026-04-23T09:18:00Z',
          body: "This is the hot path — made it `@inlinable`. Lmk if that's overkill.",
          done: false, isNew: true },
      ],
    },
  ];
  ...
  byId(5111).threads = [
    {
      id: 'th-5111-1', where: 'PlayerVM.swift L204', kind: 'review-comment',
      messages: [
        { id: 'c1', actor: 'priya.r', at: '2026-04-22T11:42:00Z',
          body: 'Heads up — this rewrites the publisher graph. I tried to keep the public surface identical but please scrutinize the test diff.',
          done: false },
      ],
    },
  ];
  ...
  byId(5105).threads = [
    {
      id: 'th-5105-1', where: 'MiniPlayerCoordinator.swift L77',
      kind: 'review-comment', kindLabel: 'Changes requested',
      messages: [
        { id: 'd1', actor: 'alex.chen', at: '2026-04-21T14:02:00Z', isMine: true,
          body: "This will leak the coordinator when we navigate back. Can you weakify the closure?" },
      ],
    },
  ];
  ...
  byId(5102).threads = [
    {
      id: 'th-5102-1', where: 'OfflineToast.swift L52',
      kind: 'review-comment', kindLabel: 'Changes requested',
      messages: [
        { id: 'e1', actor: 'alex.chen', at: '2026-04-20T09:44:00Z', isMine: true,
          body: "Let's pull the dismiss-on-scroll behavior out into the modifier so it's reusable." },
      ],
    },
  ];
  ...
  byId(5098).threads = [
    {
      id: 'th-5098-1', where: 'general', kind: 'pr-comment',
      messages: [
        { id: 'f1', actor: 'samir', at: '2026-04-23T07:12:00Z',
          body: '@alex.chen can you verify the migration path for older installs? Specifically users who skipped 5.0 and went straight to 5.2.',
          done: false, isNew: true },
      ],
    },
  ];

  // Merged PRs — fully resolved threads
  byId(5088).threads = [
    {
      id: 'th-5088-1', where: 'DownloadQueue.swift L88', kind: 'review-comment',
      messages: [
        { id: 'g1', actor: 'danieldraper', at: '2026-04-17T10:00:00Z',
          body: 'LGTM. Nice repro test.', done: true },
      ],
    },
  ];
  byId(5081).threads = [
    {
      id: 'th-5081-1', where: 'general', kind: 'pr-comment',
      messages: [
        { id: 'h1', actor: 'alex.chen', at: '2026-04-16T11:00:00Z', isMine: true,
          body: "Confirmed no callers in the iOS app. 👍" },
      ],
    },
  ];
}
...
Object.assign(window, {
  ...
  threadIsResolved, threadHasNew, threadOpenCount, prTodoCounts,
});
